    ...
    async def embedding_discord(self, embed : Embed):
        ...
        if not self.data_new_position.empty:
            for joueur, data in self.data_new_position.head(5).iterrows():
                ...
                value = format_nombre(data['value'])
                txt_24h += f'\n:arrow_up: **{data["name"]}** [{data["level"]}] ({data["shortDescription"]}) : Tu es **{position}**ème (+{dif_value}) places avec {value}'
        ...
        if not self.data_new_level.empty:
            for joueur, data in self.data_new_level.iterrows():
                ...
                if next_palier == str(0):
                    txt_level_up += f'\n:up: **{data["name"]}** ({data["shortDescription"]}) : Tu es passé **{(data["level"])}**'
                else:
                    txt_level_up += f'\n:up: **{data["name"]}** ({data["shortDescription"]}) : Tu es passé **{(data["level"])}** :arrow_right: **{next_palier}** pts pour le palier suivant'
