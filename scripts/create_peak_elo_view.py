"""Crée la vue PostgreSQL exposant le peak Elo SoloQ de chaque compte suivi.
...
La vue garde tous les comptes de ``tracker``. Un compte sans historique SoloQ
valide aura simplement les colonnes ``peak_*`` à NULL.
...
def create_peak_elo_view() -> None:
    """Crée ou remplace la vue ``public.v_peak_elo``."""
    with engine.begin() as connection:
        connection.execute(text(VIEW_SQL))


if __name__ == "__main__":
    create_peak_elo_view()
