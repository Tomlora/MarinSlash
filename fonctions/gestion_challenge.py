...
class challengeslol():
    ...
    async def comparaison(self):
        ...
        if not self.df_old_data.empty:
            ...
            self.data_comparaison = self.data_joueur.merge(self.df_old_data[['Joueur', 'challengeId', 'value_precedente', 'percentile_precedent', 'level_precedent']],
                                                        how='left',
            ...
            self.data_new_percentile = self.data_comparaison[self.data_comparaison['new_percentile'] > 0]
    ...
    async def embedding_discord(self, embed : Embed):
        ...
        if not self.data_new_level.empty:
            for joueur, data in self.data_new_level.iterrows():
                txt += f' :sparkles: **{data["name"]}** ({data["shortDescription"]}) : Tu es passé **{data["level"]}**'
