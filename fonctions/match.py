...
class matchlol():
    ...
    async def resume_general(self, name_img):
        ...
        for i in range(0, 10):
            ...
            d.text((x_dmg_percent - 20, initial_y), f'{int(self.thisDamageListe[i]/1000)}k ({int(self.thisDamageRatioListe[i]*100)}%)', font=font, fill=fill)

            fill = range_value(i, np.array(self.thisDamageTakenListe) + np.array(self.thisDamageSelfMitigatedListe)) 
        
            d.text((x_dmg_taken + 25, initial_y), f'{int(self.thisDamageTakenListe[i]/1000)}k / {int(self.thisDamageSelfMitigatedListe[i]/1000)}k', font=font, fill=fill)
