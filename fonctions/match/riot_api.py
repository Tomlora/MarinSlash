...
async def get_ranks_all_participants(session, puuid_liste):
    semaphore = asyncio.Semaphore(5)  # max 5 requêtes simultanées

    async def fetch_one(puuid):
        async with semaphore:
            ...
            if solo:
                return puuid, {'tier': solo['tier'], 'rank': solo['rank'], 'lp': solo['leaguePoints'], 'wins': solo['wins'], 'losses': solo['losses']}
            return puuid, {'tier': 'UNRANKED', 'rank': None, 'lp': 0, 'wins': 0, 'losses': 0}

    results = await asyncio.gather(*[fetch_one(p) for p in puuid_liste])
    return dict(results)
