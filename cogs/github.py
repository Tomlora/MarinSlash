            ...
            if req.status == 200:
                ...
                if len(data_user_repos) > 1:
                    ...
                    try:
                        ...
                        data_user_repos = data_user_repos[int(
                            button_ctx.data.values[0])]
                    ...
                    em.set_thumbnail(
                        url=data_user_repos['owner']['avatar_url'])
                    em.add_field(
                        name="Repository:", value=f"[{data_user_repos['name']}]({data_user_repos['html_url']})", inline=True)
                    em.add_field(name="Language:",
                                 value=data_user_repos['language'], inline=True)
                    ...
                    em.add_field(name="License:",
                                 value=license_url, inline=True)

                    if data_user_repos['stargazers_count'] != 0:
                        em.add_field(
                            name="Star:", value=data_user_repos['stargazers_count'], inline=True)

                    if data_user_repos['forks_count'] != 0:
                        em.add_field(
                            name="Fork:", value=data_user_repos['forks_count'], inline=True)

                    if data_user_repos['open_issues'] != 0:
                        em.add_field(
                            name="Issues:", value=data_user_repos['open_issues'], inline=True)
                    em.add_field(
                        name="Description:", value=data_user_repos['description'], inline=False)
