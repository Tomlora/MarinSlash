...
liste_competition_choice = [Choice(name='LEC', value='LEC'),
                                         Choice(name='LCS', value='LCS'),
                                         Choice(name='LFL', value='LFL')]
...
class Fantasy(Extension):
    ...
    @interactions.extension_command(name='fantasy_add',
                                    description='participer')
    
    async def fantasy_add(self, ctx: CommandContext):
    ...
    @interactions.extension_command(name='fantasy_bet',
                                    description="test",
                                    options=[
                                        Option(name='competition',
                                               description='Quel compétition ?',
                                               required=True,
                                               type=interactions.OptionType.STRING,
                                               choices=liste_competition_choice)]
                                    )
    async def fantasy_bet(self, ctx : CommandContext, competition:str):
        ...
        while True:
            try:
                ...
                requete_perso_bdd('''INSERT INTO fantasy_bet (id_discord, semaine, nb_match, vainqueur, competition)
                                      ...
                                      dict_params={'author_id': author_id,
                                                   'semaine': semaine,
                                                   'nb_match': button_ctx.data.custom_id[-1],
                                                   'vainqueur': button_ctx.label,
    ...
    @interactions.extension_command(name='fantasy_result',
    ...
    async def fantasy_result(self,
        ...
        liste_vainqueur = [vainqueur1, vainqueur2, vainqueur3, vainqueur4, vainqueur5]
        if isOwner_slash(ctx):
        
            df_joueurs = lire_bdd_perso('SELECT * FROM fantasy_players', index_col='id_discord').transpose()

            df_bet = lire_bdd_perso('SELECT * FROM fantasy_bet', index_col='id').transpose()
            ...
            def gestion_result(semaine:int, nb_match:int, vainqueur:str, competition:str):
                ...
                df_vainqueur = df_bet[(df_bet['semaine'] == semaine) & (df_bet['nb_match'] == nb_match) & (df_bet['competition'] == competition) & (df_bet['vainqueur'] == vainqueur)]
                
                for id_vainqueur in df_vainqueur['id_discord'].to_list():
                    new_msg += new_msg + f'**{df_joueurs.loc[id_vainqueur]["pseudo"]}** a gagné 1 point en misant pour {vainqueur} \n'
                    
                return new_msg
             
            for i, vainqueur in enumerate(liste_vainqueur):    
                msg += gestion_result(semaine, nb_match + i, vainqueur, competition)
