...
class AnalyseLoLSeason(Extension):
    ...
    @lol_analyse_season.subcommand("gold_max_min",
                            ...
                            options=[
                                SlashCommandOption(
                                    name="riot_id",
                                    description="Nom du joueur. Si discord est rempli, ce champ n'est pas utilisé",
                                    type=interactions.OptionType.STRING,
                                    required=False,
                                    autocomplete=True),
                                SlashCommandOption(
    ...
    async def gold_max_min(self,
                      ...
                      riot_tag:str = None,
                      saison = saison,
                      discord : interactions.User = None):
        ...
        FROM 
            ...
            embed = interactions.Embed(title=f'Ecart Min/Max Gold en SoloQ {riot_id}:', color=0x00FF00)
            ...
            INNER JOIN 
                ...
                ON matchs.match_id = matchs_autres.match_id
            WHERE 
                joueur in (select id_compte from tracker where discord = '{int(discord.id)}') AND season = {saison} AND mode = 'RANKED'
        ),
        max_values AS (
            SELECT 
                MAX(ecart_gold_top) AS max_top,
        ...
        FROM 
            ...
            embed = interactions.Embed(title=f'Ecart Min/Max Gold en SoloQ {discord.display_name}:', color=0x00FF00)
        ...
        for index, data in df.iterrows():
            ...
            embed.add_field(name=f'{data[0].upper().replace("_", " ")} :', value=f'  - Positif : {data[1]} ([G]({url1})) {emote_allie_max} vs {emote_ennemi_max}\n- Negatif : {data[5]} ([G]({url2})) {emote_allie_min} vs {emote_ennemi_min}', inline=False)
