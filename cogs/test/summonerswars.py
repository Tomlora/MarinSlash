...
class SummonersWars(commands.Cog):
    ...
    @main.isOwner2_slash()
    async def uploadsource(self, ctx:SlashContext, url):
        if '.json' in str(url):
    ...
    @main.isOwner2_slash()
    async def sw_general(self, ctx:SlashContext):
    ...
    @main.isOwner2_slash()
    async def sw_atk(self, ctx:SlashContext):
    ...
    @main.isOwner2_slash()
    async def sw_def(self, ctx:SlashContext):
        await ctx.defer(hidden=False)
        data = opendatasw()
    ...
    @main.isOwner2_slash()
    async def sw_base(self, ctx:SlashContext):
